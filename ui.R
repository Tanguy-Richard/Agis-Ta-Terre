  ...
  fluidPage(
    ...
    navbarPage(
      ...
      tabPanel("Avertissement et trivia",
          ...
          p("Les données des capteurs Telraam ne sont pas issues d’une mesure continue sur une heure. Pour améliorer la qualité des données futures, les capteurs dédient une partie de leur temps d’activité à l’apprentissage. Les données totales sont reconstituées à partir du temps de mesures. Plus cette période de mesure est longue plus la qualité des données est grande. Telraam donne un outil de mesure de ce temps de mesure : l’uptime. Dans cette application nous avons conservé que les données d’uptime supérieur à 0.5 (seuil conseillé par Telraam). Toutefois, les capteurs placé récemment (en période d’apprentissage) et les données matinales ou dans la soirée (visibilité réduite à cause de la nuit)  peuvent présenter des uptimes plus faible. 
          De plus la suppression des données à l’uptime trop faible fait qu’on possède moins de données pour les périodes à risque. La qualité des estimations et des test est moins bonne sur ces périodes.
          Il faut donc être prudent en interprétant ces données."),
          h3("Avertissement relatif aux catégories de mobilités :"),
          p("Les capteurs Telraam utilisés ont des difficultés à différencier les grosses voitures comme les SUV des poids lourds. Le nombre de poids lourds est donc sur-évalué et le nombre de voiture sous-évalué. Toutefois, le total voitures + camions est précis.
          ...
          p("On s’appuie sur un test de Wilcoxon Mann Whitney. L’idée est de comparer, pour chaque créneau horaire, la répartition des valeurs de chacune des périodes. Le test consiste à regarder la distance entre les fonction de répartition empirique, si elles sont éloignées, le test rejette l’hypothèse nulle :  l’égalité des lois. L’option « Significatif » indique une p-value inférieure à 0.05, celle « Entre deux » une  p-value entre 0.05 et 0.1 et celle « Non significatif » une p-value supérieure à 0.1."),
          h4("Seuil d’engorgement :"),
          p("Méthode pour tracer les courbes :"),
          p("On commence par filtrer les données selon les sélections de l’utilisateur. On isole la partie correspondant au pourcentage de conducteur dépassant chaque vitesse. On range les données dans l’ordre croissant du nombre de véhicules (voitures + camions). On a pré-lissé les données à l’aide d’une moyenne glissante d’une amplitude de 50 pour dégager un début tendance (courbe noir du graphique). A partir de cette tendance on a lissée nos données à l’aide de l’outil geom_smooth R. Ces courbes de lissages sont les courbes colorées du graphiques."),
          br(),
          p("Méthode pour trouver le seuil :"),
          p("L’objectif est de déterminé un seuil de rupture dans la courbe de lissage. Pour cela on utilise un test de Darling Erdös (dérivé du test de CUSUM). La fonction est implémenté dans le Package CPAT (Curtis Miller)."),
          ...
          On soustrait la tendance au flux totale pour avoir de données sans tendance. Pour ces données on fait une moyenne sur tout les lundis, puis les mardi, ect. Cela nous donne les valeurs associées au cycle de la semaine (second onglet). La partie restante après la soustraction du cycle hebdomadaire correspond au bruit statistique (troisième onglet)."),
          br(),
          ...
          br(),
          p('Le second indicateur est un indicateur de la proportion d’extremum commun entre les deux courbes. Pour cela on utilise la fonction « peaks » du package « synchrony ». Cette fonction compte le nombre de fois où les deux séries atteignent un maximum en même temps, puis les minimums pour ramener cela à la proportion total de pics (déterminer en sommant le nombre de maximum de la série en comptant le plus à celui de minimum).
